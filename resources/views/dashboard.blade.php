...
<x-app-layout>
    ...
    {{-- Accounts --}}
    <div class="w-11/12 py-6 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-4 lg:px-8 max-h-96 overflow-auto">
        @foreach ($accounts as $account)
            @if ($account->current_balance > 1)
                <div class="w-full py-4 px-4 rounded-md shadow-lg bg-slate-700 border-l-4" style="border-color: {{ $account->color }}">
